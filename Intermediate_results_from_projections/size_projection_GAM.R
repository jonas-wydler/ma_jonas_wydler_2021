...
res <- mclapply(files.base, function(f) {
  ...
  colnames(m.sub)[c(4,5)] <- c("species","HSI")
   # Provide species body size data to m.sub table
            ...
            #mean.size <- data.frame(m.sub %>% group_by(cell_id) %>% summarize(x = unique(x), y = unique(y), mean.size = weightedMean(x = size, w = HSI)))
            
            #mean.size2 <- data.frame(m.sub %>% group_by(cell_id) %>% summarize(x = unique(x), y = unique(y), mean.size2 = sum(HSI * size)/sum(HSI)))
            ...
            colnames(df4) <- c("cell_id","x","y","mean.size","x.2")

            g1 <- ggplot() + geom_raster(aes(x = x.2, y = y, fill = mean.size), data = df4) +
...
setwd(wd_script)
ggsave(plot = g2, filename = paste0("Copepod_nmedian size_n(mm)_map_annual_GAM_narmF.jpg"),dpi = 300, width = 7, height = 4)


setwd(wd_ensemble)
annual_size_GAM_naF <- med.size2
save(annual_size_GAM_naF,file = "annual_size_GAM_naF")

# -------------------------------------------------------------------------------------------------
#do annual ensemble calculations
# setwd(wd_ensemble)
...
# 
# 
# size_ensm <- bind_rows(annual_size_GAM_naF,annual_size_GLM_naF,annual_size_ANN_naF)
# 
# df_size_ensm <- data.frame(size_ensm %>% group_by(cell_id) %>% 
#                            summarize(x = unique(x), y = unique(y), med.size = mean(med.size, na.rm = T)))
# 
# df <- df_size_ensm
# df$x.2 <- df$x
# df[df$x < 0 ,"x.2"] <- (df[df$x < 0 ,"x"]) + 360
...
# 
# setwd(wd_script)
# ggsave(plot = g2, filename = paste0("Copepod_nmedian size_n(mm)_map_annual_ensemble_narmF.jpg"),dpi = 300, width = 7, height = 4)
