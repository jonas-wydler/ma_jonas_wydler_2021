...
res <- mclapply(files, function(f) {
  ...
  #transform to 1/0
  species_tss_names <- unique(df_GLM_v2$species)
  ...
  df_GLM_v2_subset <- subset(df_GLM_v2, is.element(df_GLM_v2$species,common.spp2))
  
  #sorting
  df_GLM_v2_subset$species <- as.character(df_GLM_v2_subset$species)
  df_GLM_v2_subset <- df_GLM_v2_subset[order(df_GLM_v2_subset$species),]
  ...
  #df_GLM_v2_subset$species == colnames(subset[4:length(subset)]) #this is to test
  
  
  df_GLM_v2_subset$tss_cutoff <- df_GLM_v2_subset$tss_cutoff/1000
  #subset2 <- subset #subset2 is used for binary ct
  subset[4:length(subset)] <- BinaryTransformation(subset[4:length(subset)],df_GLM_v2_subset$tss_cutoff )
  ...
  ex2 <- pd(abun, as.phylo(fit_famd_ward), include.root=TRUE)
  ...
  g1 <- ggplot() + geom_raster(aes(x = x.2, y = y, fill = PD), data = df) +
    ...
    scale_fill_viridis(na.value = "white") + labs(fill = "FR (Faith index)") 
    #scale_fill_gradient2(name = "FR",space = "Lab", mid = "white", low = "#3288BD", high = "#D53E4F", na.value = "white", aesthetics = "fill",guide = "colourbar")
    #scale_fill_brewer(palette = "PuOr")
  setwd(wd_script)
  ggsave(plot = g1, filename = paste0("FR_binary_GLM_",name_month,"_famd_ward_v2.jpg"),dpi = 300, width = 7, height = 4)
  ...
  # 
  # ggsave(plot = g2, filename = paste0("Quantiles_binary_famd_ward_",name_month,"_GLM_v2.jpg"),dpi = 300, width = 7, height = 4)
  save(df, file = paste0("dat_binary_GLM_", name_month,"_famd_ward_v2.Rda"))
...
#test <- res[[1]]
save(table_df, file = paste0("dat_faith_annual_GLM_binary_famd_ward_v2.Rda"))

table_df_an <- data.frame(table_df %>% group_by(cell_id) %>% 
                               summarize(x = unique(x), y = unique(y), PD = mean(PD, na.rm = T)))
df2 <- table_df_an
df2$x.2 <- df2$x
df2[df2$x < 0 ,"x.2"] <- (df2[df2$x < 0 ,"x"]) + 360
#Here I rename the month to filler so that it works with changing names

g1 <- ggplot() + geom_raster(aes(x = x.2, y = y, fill = PD), data = df2) +
  # play around with palettes. You can also use scale_fill_distiller() and
  geom_polygon(aes(x = long, y = lat, group = group), data = world2, fill = "black", colour = "black", size = 0.3) +
  coord_quickmap() + theme(panel.background = element_rect(fill = "white"),legend.key = element_rect(fill = "grey50"),
                           panel.grid.major = element_line(colour = "grey70",linetype = "dashed"), legend.position = "bottom") +
...
#setwd(wd_maps)
ggsave(plot = g1, filename = paste0("FR_binary_annual_GLM_famd_ward_v2.jpg"),dpi = 300, width = 7, height = 4)
...
# 
# ggsave(plot = g4, filename = paste0("Quantiles_binary_annual_GLM_ward.jpg"),dpi = 300, width = 7, height = 4)
