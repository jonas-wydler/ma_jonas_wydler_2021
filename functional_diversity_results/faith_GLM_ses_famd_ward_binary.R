...
res <- mclapply(files, function(f) {
  ...
  fit_famd_avg <- hclust(famd_dist, method = "ward.D2")
  dev.off()
  # -------------------------------------------------------------------------------------------------
  
  
  #ex2 <- pd(abun, as.phylo(fit_famd_avg), include.root=TRUE)
  ex2 <- ses.pd(samp = abun, tree = as.phylo(fit_famd_avg), null.model = "taxa.labels", runs = 100, include.root = T)
  ...
  setwd(wd_script)
  ggsave(plot = g1, filename = paste0("FR_ses_binary_GLM_",name_month,"_famd_avg_v2.jpg"),dpi = 300, width = 7, height = 4)
  ...
  ggsave(plot = g2, filename = paste0("Quantiles_binary_famd_avg_",name_month,"_GLM_v2.jpg"),dpi = 300, width = 7, height = 4)
  save(df, file = paste0("ses_dat_binary_GLM_", name_month,"_famd_avg_v2.Rda"))
...
#test <- res[[1]]
save(table_df, file = paste0("ses_dat_faith_annual_GLM_binary_famd_avg_v2.Rda"))
...
ggsave(plot = g3, filename = paste0("FR_ses_binary_annual_GLM_famd_avg_v2.jpg"),dpi = 300, width = 7, height = 4)
...
#   theme(legend.title = element_text(size=8))
# 
# ggsave(plot = g4, filename = paste0("Quantiles_binary_annual_GLM_avg.jpg"),dpi = 300, width = 7, height = 4)
